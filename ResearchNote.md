...
### 研究内容
#### 前回のまとめ
* Pitch角のみでRing Currentとして議論ができるのか？
  * 先行研究：「ピッチ角90度近辺のフラックスがある→Ring Current」
  * エネルギー分解能がないため，議論ができない？

#### 本郷セミナーについて
* 現在考えている電流以外に成因はないのだろうか
  * 例えば，Region2が原因でも同様なことは生じないのか？
* 「Ring Currentが原因として，定義は自分でしてくれ」

### 作業内容
#### Pitch角異方性について
各軌道について，最大のフラックスで規格化したうえでフラックスとピッチ角の関係の議論を行ってみる．

以下作成した図

<img src='./figure/Pchang/flux_mean_all_normalized.png' width='80%'>

結論から言うと
**どの場合でもピッチ角90度に強いフラックスがある**
ということになった．

* 場所・状態にかかわらず，常にピッチ角90度近辺に強いフラックスがある
  * どのような条件下でも常に90度近辺に強いフラックスが生じている
  * NSのあたりの領域でも異方性がある？
  * 温度や有無などはフラックスの異方性には依存しない？
  * 圧力とは関係してそう...
* ピッチ角からRing Currentか否かの議論をするのはかなり難しい？

#### Region2について
磁場方向ベクトルとの内積をとって磁場に沿った方向への電流がどれくらいかを見てみたい

...が，そもそもRegion2があるか否かの議論はMESSENGERがdawn-dusk方向へ移動していないと議論が出来ないのでは？

ともかく，計算をしてみる．
### 今後の予定
* Region2の強度の研究は今までのアプローチと別になる．
  * Dipのデータのみで議論するのは難しい
  * Pressure gradient currentとつじつまが合う（ことが示せれば）→FACを持ち出す必要は（とりあえず）無し
    * Pressure gradientでどれだけ説明ができるのかをやってみる
  * 地球の場合：Current wedgeはデータの積み重ねで議論
    * 優先度は低いが地球の論文を読んでみる
    * 水星でも論文はあったはずなので，これも読んでみる
  * Ring Currentよりもオーダーで小さいはず...「寄与はない」と説明できそう
  * 修士論文で結論をつけるのは難しい（discussionするのはアリ）
* Pitch核について
  * 昼側領域でも90度に多いのかを確認してみる
* pressure gradientでの説明をしてみる（ケーススタディーだけではなく，全体で）
  * **本当にそこまで下がるのかを説明！！！**
...
### 研究内容
#### 前回のまとめ
Pitch角分布で議論をするのは難しそう...
* どのタイプ（depressionがあってもなくても）90どのあたりに多く集中しているように見える
  * 90度の値で規格化してみる
  
圧力勾配から議論してみる
* 反磁性電流の計算から，rotB電流の大きさを説明できるのか
  * とりあえず，電流の大きさで比較してみる
  * 構造についても議論していきたい
* 圧力からdepressionの説明ができれば御の字
 
### 作業内容
#### pitch角について
90度のfluxで規格化してみる
<img src='./figure/Pchang/flux_mean_all_normalized_90degree.png' width='80%'>

前回と同様，90度のFluxはどのタイプでも同様に強いが，
dipタイプ（特に25MK以上）のものでシャープに出ている   
→**dipタイプではpitch角90度のFluxが卓越していると言えそう？**

NSタイプでanti-parallelのFluxが強いのは機器の特徴？それともこの領域の特徴？

#### 圧力について
depressionの成因が反磁性電流によるものであることを示したい
* 反磁性電流の大きさがrotB電流に比べてどうなっているのかを考えてみる
  * 例えばそれぞれの電流の最大値・最小値を比較してみる？
    * 空間的依存も考えに入れなければいけない
    * 例えば単純に最大値が等しい場合にも，空間的に異なる場所で生じているなら，単純比較しても意味がない？
  * dawn-wardな電流を考えることが重要？
    * 圧力が等方であることを仮定すると反磁性電流は $\mathbf{j}_{diamag}=\frac{\mathbf{B}}{B^2}\times\mathbf\nabla P$
    * dawn-to-dusk電流は$j_y = B_x\frac{\partial P}{\partial z}-B_z\frac{\partial P}{\partial x}$
    * 尾部領域では$B_Z\simeq0$，$B_X$が反転する領域に高圧領域があるならば，第一項の符号は常に正になるはず→duskwardな電流のみが生じるはず
  
とりあえず，各電流の最大値・最小値のみを比較してみた．以下が結果の図

<img src='./figure/Current/diamag_rotB_dawnward.png' width='80%'>
<img src='./figure/Current/diamag_rotB_duskward.png' width='80%'>

理想的には，特にduskwardについて，
* dipタイプのほうがrotBが大きくなるにつれてdiamagが大きくなる
* nsタイプではdiamagがほとんど0で変化なし
を期待していたがそこまできれいではない．とはいえ，定性的にはdipタイプのほうが
反磁性電流が強いと言えそう

diamagのみに注目したものが以下の図
<img src='./figure/Current/diamag_hist_dawnward.png' width='80%'>
<img src='./figure/Current/diamag_hist_duskward.png' width='80%'>
定性的に，dipタイプのほうがdiamagが強いことが言えそうである．   
さらに，温度も絡めた図が以下の通り
<img src='./figure/Current/diamag_hist_temp_dawnward.png' width='80%'>
<img src='./figure/Current/diamag_hist_temp_duskward.png' width='80%'>
少しわかりにくいが，dipタイプの高温のものについて，他のタイプより反磁性電流の強さが1桁大きいことがわかる．

これらから，特にdipタイプの高温のものでは，反磁性電流の効果が強く効いていそうといえる？

depressionの窪みの深さとdiamagの強さなどとの比較だったり，pressure gradientの空間スケールと絡めて議論したい
### 今後の予定
### 作業ファイル